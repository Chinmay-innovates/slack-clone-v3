...
export const WorkspaceSwitcher = () => {
  ...
  const { workspace, setWorkspace, otherWorkspaces, setOtherWorkspaces, setChannel } =
  ...
  const switchWorkspace = (otherWorkspace: Workspace) => {
    setOtherWorkspaces([...otherWorkspaces.filter((w) => w.id !== otherWorkspace.id), workspace]);
    setWorkspace(otherWorkspace);
    setChannel(otherWorkspace.channels[0]);
    router.push(`/client/${otherWorkspace.id}/${otherWorkspace.channels[0].id}`);
  };
    ...
    >
      ...
      >
        <div className="w-full px-4 py-2 text-[15px] leading-7 hover:bg-[#36383b]">
          <div className="leading-[22px] font-bold truncate">{workspace.name}</div>
        ...
        <div className="flex flex-col text-[12.8px] leading-[1.38463] m-[4px_12px_4px_16px]">
          ...
          <div>
            <span className="cursor-pointer text-[#1D9BD1] hover:underline">Get the Slack app</span>{' '}
        ...
        {otherWorkspaces.map((otherWorkspace) => (
          ...
          >
            ...
            <div className="flex flex-col text-left">
              <div className="leading-[22px] font-bold truncate">{otherWorkspace.name}</div>
              <div className="text-[13px] leading-[18px]">
        ...
        >
          ...
          <div className="flex flex-col text-left text-white">Add a workspace</div>
        </button>
      </div>
    </div>
  );
