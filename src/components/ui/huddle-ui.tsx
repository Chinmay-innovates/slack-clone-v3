import { StreamTheme, PaginatedGridLayout, SpeakerLayout } from '@stream-io/video-react-sdk';
    ...
    <StreamTheme className={clsx('huddle', isSidebar && 'huddle--sidebar')}>
      ...
      >
        ...
        >
          ...
          >
            <div className={clsx('flex items-center', isSidebar ? 'gap-1.5' : 'gap-2')}>
              <CallControlButton
                icon={isMicrophoneMute ? <MicrophoneOff size={20} /> : <Microphone size={20} />}
              ...
              {width > 0 && width >= 300 && (
                <CallControlButton icon={<Emoji size={20} />} title={'Send a reaction'} />
              )}
              {width > 0 && width >= 340 && (
                <CallControlButton icon={<UserAdd size={20} />} title={'Invite people'} />
              )}
              <CallControlButton icon={<MoreVert size={20} />} title={'More options'} />
            </div>
            <button
              onClick={() => {
                leaveCall();
